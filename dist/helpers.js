...
export const metaCriticGames = async () => {
    const response = await fetchRequest("https://internal-prod.apigee.fandom.net/v1/xapi/finder/metacritic/web?sortBy=-metaScore&productType=games&gamePlatformIds=1500000129&page=2&releaseYearMin=1958&releaseYearMax=2024&offset=24&limit=24&apiKey=1MOZgmNFxvmljaQR1X9KAij9Mo4xAY3u", "https://www.metacritic.com/");
    let json = await response.json();
    let items = [...json.data.items];
    while (json.links.next?.href) {
        const response = await fetchRequest(json.links.next.href, "https://www.metacritic.com/");
...
export const xboxGamePassIds = async () => {
    const response = await fetchRequest("https://catalog.gamepass.com/sigls/v2?id=f6f1f99f-9b49-4ccd-b3bf-4d9767a77f5e&language=en-ca&market=CA", "https://www.xbox.com/");
    let json = await response.json();
...
export const xboxGamePassCatalogUrls = async () => {
    ...
    return chunks.map((chunk) => `https://displaycatalog.mp.microsoft.com/v7.0/products?bigIds=${chunk.join(",")}&market=CA&languages=en-ca`);
};
/**
 * Xbox Game Pass Game
 * @function
...
export const xboxGamePassGame = (game) => {
    const slugify = (str) => str
        ...
        .replace(/[\s_-]+/g, "-")
        .replace(/^-+|-+$/g, "");
    const [score, scoreCount] = game.MarketProperties[0].UsageData.reduce((acc, val) => {
    ...
    return {
        ...
        url: `https://www.xbox.com/en-ca/games/store/${slugify(game.LocalizedProperties[0].ProductTitle)}/${game.ProductId}`,
        score,
        scoreCount,
        // XblLocalCoop,
...
export const puppeteerGetHtml = async (url) => {
    ...
    await page.setUserAgent("Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Mobile Safari/537.36");
    await page.goto(url);
